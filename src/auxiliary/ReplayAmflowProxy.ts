			...
			this._amflow.getTickList(from, to, (err: Error | null, tickList?: pl.TickList) => {
				...
				if (!tickList) {
					...
					if (!fromInGiven && !toInGiven) {
						...
						let ticksWithEvents =
							this._sliceTicks(givenTicksWithEvents, from, to).concat(tickList[EventIndex.TickList.TicksWithEvents] || []);
						callback(null, [from, tickList[EventIndex.TickList.To], ticksWithEvents]);

					} else { // 後半重複
						let ticksWithEvents =
							(tickList[EventIndex.TickList.TicksWithEvents] || []).concat(this._sliceTicks(givenTicksWithEvents, from, to));
