...
describe("StorageResolver", function () {
	...
	it("can be instantiated", function () {
		...
		expect(self.errorTrigger.isHandled(prepared.errorCollector, prepared.errorCollector.add)).toBe(true);
	...
	it("can switch execution mode", function () {
		...
		expect(self._executionMode).toBe(ExecutionMode.Active);
		expect(self._tickGenerator.gotStorageTrigger.isHandled(self, self._onGotStorageOnTick)).toBe(true);
		expect(self._tickBuffer.gotStorageTrigger.isHandled(self, self._onGotStorageOnTick)).toBe(false);

		self.setExecutionMode(ExecutionMode.Passive);
		expect(self._executionMode).toBe(ExecutionMode.Passive);
		expect(self._tickGenerator.gotStorageTrigger.isHandled(self, self._onGotStorageOnTick)).toBe(false);
		expect(self._tickBuffer.gotStorageTrigger.isHandled(self, self._onGotStorageOnTick)).toBe(true);
